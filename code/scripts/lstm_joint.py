...
fh = logging.FileHandler(os.path.join(model_dir, 'lstm_joint.' + str(random_seed) + '.log'), mode='w')
...
class ICSL(Block):
    ...
    The model feeds token ids into a biLSTM to get the sequence
    representations, then apply two dense layers for IC/SL task.
    """

    def __init__(self, vocab_size, num_slot_labels, num_intents, embed_size=256, rnn_hidden_size=128, rnn_layers=1, rnn_dropout=.1, embed_dropout=.1, prefix=None, params=None):
        super(ICSL, self).__init__(prefix=prefix, params=params)
        with self.name_scope():
            # Embedding
            self.word_embed = nn.Embedding(input_dim=vocab_size, output_dim=embed_size)
            self.embed_dropout = nn.Dropout(rate=embed_dropout)
            # RNN encoder
            self.rnn = rnn.LSTM(rnn_hidden_size, num_layers=rnn_layers, bidirectional=True, dropout=rnn_dropout)
            # IC/SL classifier
    ...
    def forward(self, inputs):  # pylint: disable=arguments-differ
        ...
        embed = self.word_embed(inputs)
        embed = self.embed_dropout(embed)
        # hidden: (seq_length, batch_size, rnn_hidden_size * 2)
        hidden = self.rnn(embed.swapaxes(0, 1))
        # hidden: (batch_size, seq_length, rnn_hidden_size * 2)
...
# Train
log.info('Train on all languages:')
model_name = 'model_lstm_joint.' + str(random_seed)
train_input = data_dir + 'atis_train_all.tsv'
